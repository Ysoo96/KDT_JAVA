...
public class BoardDeleteController {
	...
	public String deleteProc(@RequestParam("boardNum") int boardNum,
						     @RequestParam("boardPass") String boardPass,
						     Model model) {

		log.info("------ deleteProc : boardNum : {}, boardPass : {}", boardNum, boardPass);
		...
		String movePage = "/board/view/" + boardNum; // 리턴(이동) 페이지
		...
		if (boardService.selectBoardsCountWithReplies(boardNum) > 0) { // 댓글(들)이 있을 경우
			...
			if (boardPass.trim().equals(boardVO.getBoardPass())) {
				...
				if (boardService.deleteById(boardNum) == true) {
					...
					msg = "게시글 삭제에 성공하였습니다.";
					movePage = "/board/list"; // 게시글 목록으로 인동
